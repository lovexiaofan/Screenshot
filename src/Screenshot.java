...
public class Screenshot extends JFrame implements ActionListener {
    ...
    private void initWindow(){
        ...
        JLabel jl1=new JLabel("作者: Guo",JLabel.CENTER);
     ...
     *处理把当前的图片加入剪帖板的方法
     */
    private void doCopy(final BufferedImage image){
        try{
            if(get==null){
                JOptionPane.showMessageDialog(this
                        ,"图片不能为空!!","错误",JOptionPane.ERROR_MESSAGE);
    ...
    //保存BMP格式的过滤器
    private static class BMPfilter extends javax.swing.filechooser.FileFilter{
        BMPfilter(){
        }
    ...
    //保存JPG格式的过滤器
    private static class JPGfilter extends javax.swing.filechooser.FileFilter{
        JPGfilter(){
        }
        public boolean accept(File file){
    ...
    //保存GIF格式的过滤器
    private static class GIFfilter extends javax.swing.filechooser.FileFilter{
        GIFfilter(){
        }
        public boolean accept(File file){
    ...
    //保存PNG格式的过滤器
    private static class PNGfilter extends javax.swing.filechooser.FileFilter{
        public boolean accept(File file){
    ...
    private class Temp extends JPanel implements MouseListener, MouseMotionListener {
        ...
        static final int END_X=3;
        static final int END_Y=4;
        ...
        Temp(JFrame jf, BufferedImage bi, int width, int height){
            this.jf=jf;
            this.bi=bi;
            this.width=width;
            this.height=height;
